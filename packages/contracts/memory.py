...
class Memory(BaseModel):
    ...
    surface: Optional[str] = Field(default=None, description="Surface this memory is eligible for; None = all")


class MemoryCorrectionRequest(BaseModel):
    """Input for PATCH /v1/memories/{memory_id} — correct, supersede, or expire."""
    action: Literal["correct", "expire", "reactivate"]
    new_fact_text: Optional[str] = Field(default=None, description="Required when action == 'correct'")
